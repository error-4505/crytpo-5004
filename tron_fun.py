""" TTk45PWMqYkQyy5YTUeAPGEPpRwM6bt9Wq - coffee donation for the developer """
...
logging.basicConfig(level=logging.INFO, filename='stron.log', format="%(asctime)s %(levelname)s %(message)s")
        ...
        while True:
            ...
            if txid != f_txid:
                if amount > 1:
                    ...
                    # info
                    print("Новая транзакция", trx, "TRX", txid, "txID")
...
def commission_set():
    ...
    scaled_number = bal * 10**6  # Умножаем на 10^6 для получения 6 знаков после запятой
    rounded_number = round(scaled_number)  # Округляем число
    int_number = int(rounded_number)  # Приводим к типу int
...
def send_multi_sign_trx(amount):
    ...
    try:
        ...
        if id_tx is not None:
            # info
            print(f"Транзакция успешно отправлена: {trx} TRX: {id_tx} txID")
...
def err_new_trans():
    ...
    for admin in admins_id:
        ...
        tg.notify(token=bot_key, chat_id=admin,
                  message=f"Ошибка при получении новой транзакции,смотрите log файл")
...
def good_new_trans(trx, txid):
    ...
    for admin in admins_id:
        tg = get_notifier('telegram')
        tg.notify(token=bot_key, chat_id=admin,
                  message=f"Новая транзакция успешно получена: {trx} TRX: {txid}")
...
def err_send_trans():
    ...
    for admin in admins_id:
        tg = get_notifier('telegram')
        tg.notify(token=bot_key, chat_id=admin,
                  message=f"Ошибка при отправки транзакции, смотрите log файл")
...
def good_send_trans(trx, id_tx):
    ...
    for admin in admins_id:
        tg = get_notifier('telegram')
        tg.notify(token=bot_key, chat_id=admin,
                  message=f"Транзакция успешно отправлена: {trx} TRX: {id_tx}")
